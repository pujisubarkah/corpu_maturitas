...
import { kategori } from '../../../lib/schemas/kategori'
...
// GET: list all grouped by kategori or get by id (query ?id=)
export async function GET(request: Request) {
  try {
    ...
    // Get all pertanyaan with kategori join
    const rawData = await db
      .select({
        ...
        // Pertanyaan fields
        id: pertanyaanKompetensi.id,
        kode: pertanyaanKompetensi.kode,
        pertanyaan: pertanyaanKompetensi.pertanyaan,
        deskripsi: pertanyaanKompetensi.deskripsi,
        tipe_jawaban: pertanyaanKompetensi.tipe_jawaban,
        urutan: pertanyaanKompetensi.urutan,
        is_required: pertanyaanKompetensi.is_required,
        created_at: pertanyaanKompetensi.created_at,
        updated_at: pertanyaanKompetensi.updated_at,
      })
      .from(pertanyaanKompetensi)
      .leftJoin(kategori, eq(pertanyaanKompetensi.kategori_id, kategori.id))

    // Group by kategori
    const groupedData = rawData.reduce((acc: GroupedData[], item) => {
      const existingKategori = acc.find(k => k.kategori.id === item.kategori_id)
      ...
      if (existingKategori) {
        existingKategori.pertanyaan.push(pertanyaanItem)
      } else {
        acc.push({
          kategori: {
            id: item.kategori_id,
            nama: item.kategori_nama,
            deskripsi: item.kategori_deskripsi,
          },
