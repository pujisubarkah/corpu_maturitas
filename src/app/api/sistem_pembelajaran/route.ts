...
import { sistemPembelajaran, surveiCorpu } from "../../../lib/schemas/profile_surveys";
...
export async function GET(request: NextRequest) {
  ...
  if (userId && tahun) {
    ...
    // Then find the sistemPembelajaran record
    const data = await db.select().from(sistemPembelajaran).where(
      eq(sistemPembelajaran.surveiId, surveiRecord[0].id)
    );
    return NextResponse.json({ data });
  } else {
...
export async function PUT(req: NextRequest) {
  ...
  const updated = await db.update(sistemPembelajaran).set(updateData).where(eq(sistemPembelajaran.surveiId, surveiId)).returning();
...
export async function POST(req: NextRequest) {
  ...
  // Check if sistemPembelajaran record exists
  const existing = await db.select().from(sistemPembelajaran).where(
    eq(sistemPembelajaran.surveiId, surveiId)
  );
  ...
  if (existing.length > 0) {
    ...
    const updated = await db.update(sistemPembelajaran)
      .set(data)
      .where(eq(sistemPembelajaran.surveiId, surveiId))
      .returning();
    return NextResponse.json(updated[0]);
  } else {
    // Insert new record
    const inserted = await db.insert(sistemPembelajaran).values({ surveiId, ...data }).returning();
