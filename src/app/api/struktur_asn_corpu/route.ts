...
import { strukturAsnCorpu, surveiCorpu } from "../../../lib/schemas/profile_surveys";
...
export async function GET(request: NextRequest) {
  ...
  if (userId && tahun) {
    ...
    // Then find the strukturAsnCorpu record
    const data = await db.select().from(strukturAsnCorpu).where(
      eq(strukturAsnCorpu.surveiId, surveiRecord[0].id)
    );
    return NextResponse.json({ data });
  } else {
...
export async function PUT(req: NextRequest) {
  ...
  const updated = await db.update(strukturAsnCorpu).set(updateData).where(eq(strukturAsnCorpu.surveiId, surveiId)).returning();
...
export async function POST(req: NextRequest) {
  ...
  // Check if strukturAsnCorpu record exists
  const existing = await db.select().from(strukturAsnCorpu).where(
    eq(strukturAsnCorpu.surveiId, surveiId)
  );
  ...
  if (existing.length > 0) {
    ...
    const updated = await db.update(strukturAsnCorpu)
      .set(data)
      .where(eq(strukturAsnCorpu.surveiId, surveiId))
      .returning();
    ...
    // Insert new record
    const inserted = await db.insert(strukturAsnCorpu).values({ surveiId, ...data }).returning();
